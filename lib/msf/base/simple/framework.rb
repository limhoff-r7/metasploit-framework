...
module Framework
  ...
  def self.simplify(framework, options={})
    options.assert_valid_keys('DeferModuleLoads', 'DisableLogging', 'OnCreateProc')
    ...
    unless options['DisableLogging']
      Msf::Logging.setup!(
          logs_pathname: framework.pathnames.logs
      )
    end
  ...
  #
  def load_config
    data_store.from_file(pathnames.file, 'framework/core')
  end
  ...
  #
  def save_config
    data_store.to_file(pathnames.file, 'framework/core')
  end
