...
class EXE
  ...
  def self.read_replace_script_template(filename, hash_sub)
    template_pathname = Metasploit::Framework.pathnames.data.join('templates', 'scripts', filename)
    template = ''
  ...
  def self.to_win64pe(options={})
    ...
    options[:exe_type] = :exe_sub
    exe_sub_method(code, options)
  end
...
<dict>
  ...
  def self.to_jar(exe, opts={})
    ...
    zip.add_files(paths, Metasploit::Framework.pathnames.java_classes)
